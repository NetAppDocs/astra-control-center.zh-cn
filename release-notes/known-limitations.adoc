...
如果您想在另一个 Astra Control Center 实例上管理某个集群，应先从管理该集群的实例中将其link:../use/unmanage.html#unmanage-a-cluster["取消管理集群"]，然后再在另一个实例上对其进行管理。将集群从管理中移除后，通过执行以下命令验证该集群是否已取消管理：
...
. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy.html["安装最新版本的Astra Trdent"^]。
...
请确保在建立连接时 link:../use/monitor-protect.html#add-a-proxy-server-for-connections-to-the-netapp-support-site["输入正确的值"]。
