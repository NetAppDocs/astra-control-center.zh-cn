...
+
** 对于 GKE- 集群， Astra Control Service 使用 https://["适用于 Google Cloud 的 NetApp Cloud Volumes Service"^] 或 Google Persistent Disk 作为永久性卷的存储后端。
** 对于 AKS 集群， Astra Control Service 使用 https://["Azure NetApp Files"^] 或Azure受管磁盘作为永久性卷的存储后端。
...
** 如果您使用NetApp云服务存储产品作为存储后端、则Astra Control Service将使用Astra Trident为应用程序配置永久性卷。如果您使用Amazon EBS或Azure托管磁盘作为存储后端、则需要安装特定于提供商的CSI驱动程序。中提供了安装说明 https://["设置Amazon Web Services"^] 和 https://["使用 Azure 受管磁盘设置 Microsoft Azure"^]。
