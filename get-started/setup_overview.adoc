...
现在、您已确认满足了这些前提条件、您已做好准备  cluster,添加集群。
...
.您需要什么？ #8217 ；将需要什么
* 在添加集群之前，请查看并执行必要的操作  your environment for cluster management using Astra Control,前提条件任务。
...
IMPORTANT: 创建自己的 `kubeconfig` file中、您只能定义*一*上下文元素。请参见 https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/["Kubernetes 文档"^] 有关创建的信息 `kubeconfig` 文件。如果您使用为有限集群角色创建了kubeconfig  a limited cluster role kubeconfig,上述过程、请务必在此步骤中上传或粘贴kubeconfig。
