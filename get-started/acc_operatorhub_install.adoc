...
. [substep_image_local_registry_push]]将Astra控制中心映像目录中的软件包映像推送到本地注册表。在运行之前、请进行以下替换 `push-images` 命令：
+
...
+
image:openshift_operatorhub.png["Astra 控制中心安装页面"]
...
+
image:red_hat_catalog.png["Astra 控制中心概述页面"]
...
. 从Astra Control Center操作员的* Astra Control Center*选项卡中的控制台中、选择*创建AstraControlCenter*。image:openshift_acc-operator_details.png["Astra控制中心操作员页面"]
...
NOTE: 有关CRD选项的详细信息、请参见 https://docs.netapp.com/us-en/astra-control-center/get-started/acc_cluster_cr_options.html["本节"^]。
