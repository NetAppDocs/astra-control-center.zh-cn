...
* * Astra Trident / ONTAP 存储配置 * ： Astra 控制中心要求已安装并配置 Astra Trident 21.04 或更高版本，以便使用 NetApp ONTAP 9.5 或更高版本作为存储后端。Astra 控制中心要求创建一个存储类并将其设置为默认存储类。Astra 控制中心支持由 Astra Trident 提供的以下 ONTAP 驱动程序：
+
** ontap-NAS
** ontap-nas-flexgroup
** ontap-san
