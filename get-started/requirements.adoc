...
+
** 默认 VMware TKG 和 TKGi 配置文件令牌在部署后 10 小时过期。如果您使用 Tanzu 产品组合，则必须生成包含不过期令牌的 Tanzu Kubernetes 集群配置文件，以防止 Astra Control Center 与托管应用程序集群之间出现连接问题。有关说明，请访问  https://techdocs.broadcom.com/us/en/vmware-cis/nsx/vmware-nsx/4-2.html["VMware NSX-T Data Center 产品文档"^]。
