...
+
** 对于 GKE 集群，Astra Control Service 使用 https://www.netapp.com/google-cloud/netapp-volumes/["适用于 Google Cloud 的 NetApp Cloud Volumes Service"^]或 Google Persistent Disks 作为永久卷的存储后端。
** 对于 AKS 集群，Astra Control Service 使用 https://www.netapp.com/azure/azure-netapp-files/["Azure NetApp Files"^]或 Azure 托管磁盘作为永久卷的存储后端。
